...
function Navbar() {
  return (
    <div style={{ padding: "24px 32px", borderBottom: "1px solid rgba(250,204,21,0.2)", background: "linear-gradient(to right,rgba(15,23,42,0.5),rgba(30,41,59,0.5))", display: "flex", alignItems: "center", justifyContent: "space-between", boxSizing: "border-box", width: "100%" }}>
      <Link to="/" style={{ fontSize: "24px", fontWeight: 900, letterSpacing: "2px", background: "linear-gradient(90deg,#fde68a,#facc15,#f59e0b)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textDecoration: "none", flexShrink: 0 }}>NEURIAL</Link>
      <div style={{ display: "flex", gap: "24px", alignItems: "center" }}>
        {[["Features","/features"],["Pricing","/pricing"],["Use Cases","/use-cases"]].map(([label, path]) => (
          <Link key={path} to={path} style={{ color: "rgba(254,240,138,0.7)", textDecoration: "none", fontSize: "14px", whiteSpace: "nowrap" }}>{label}</Link>
        ))}
        <Link to="/app" style={{ padding: "8px 20px", borderRadius: "12px", fontWeight: 700, fontSize: "14px", background: "linear-gradient(to right,#facc15,#fde047)", color: "#000", textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>Open App</Link>
      </div>
    </div>
  );
}
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "64px 32px", textAlign: "center" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", textAlign: "left" }}>
          <div style={{ padding: "32px", borderRadius: "20px", background: "rgba(30,41,59,0.7)", border: "2px solid rgba(71,85,105,0.5)" }}>
            ...
            <p style={{ color: "#475569", fontSize: "28px", fontWeight: 900, margin: "0 0 24px 0" }}>0 euros</p>
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "10px" }}>
              {[["v","10 minute sessions"],["v","All noise colors"],["v","Nature sounds"],["x","No audio export"],["x","No commercial use"]].map(([icon, text]) => (
                <li key={text} style={{ display: "flex", gap: "10px", fontSize: "14px" }}>
                  <span style={{ color: icon === "x" ? "#f87171" : "#64748b", flexShrink: 0 }}>{icon}</span>
                  <span style={{ color: icon === "x" ? "#475569" : "#94a3b8" }}>{text}</span>
                </li>
              ))}
            </ul>
          </div>
          <div style={{ padding: "32px", borderRadius: "20px", position: "relative", background: "rgba(250,204,21,0.08)", border: "2px solid rgba(250,204,21,0.5)", boxShadow: "0 8px 32px rgba(250,204,21,0.1)" }}>
            ...
            <p style={{ color: "#facc15", fontSize: "28px", fontWeight: 900, margin: "0 0 24px 0" }}>9.99 euros/month</p>
            <ul style={{ listStyle: "none", padding: 0, margin: "0 0 28px 0", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["Unlimited sessions","Unlimited audio export","WAV 24-bit and MP3 up to 320kbps","Up to 8 hours per export","Commercial use allowed","Royalty-free audio"].map((text) => (
                <li key={text} style={{ display: "flex", gap: "10px", fontSize: "14px", color: "#e2e8f0" }}>
                  <span style={{ color: "#facc15", flexShrink: 0 }}>v</span>
                  {text}
                </li>
              ))}
            </ul>

    ...
    <div style={{ width: '100vw', height: '100vh', background: 'linear-gradient(135deg,#020617,#0f172a,#020617)', fontFamily: 'system-ui,sans-serif', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 40px', gap: '16px' }}>
        <div style={{ textAlign: 'center' }}>
          <h1 style={{ fontSize: '42px', fontWeight: 900, margin: '0 0 6px 0', background: 'linear-gradient(to right,#fef9c3,#fde047,#facc15)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Features</h1>
          <p style={{ color: 'rgba(254,240,138,0.6)', margin: 0, fontSize: '14px' }}>Everything you need to create the perfect audio environment.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', width: '100%', maxWidth: '900px' }}>
          {FEATURES.map(([icon, title, desc]) => (
            <div key={title} style={{ padding: '14px 16px', borderRadius: '10px', background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(250,204,21,0.2)', display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
              <span style={{ fontSize: '22px', flexShrink: 0, marginTop: '1px' }}>{icon}</span>
              <div>
                <h3 style={{ color: '#fef08a', fontWeight: 700, fontSize: '13px', margin: '0 0 5px 0' }}>{title}</h3>
